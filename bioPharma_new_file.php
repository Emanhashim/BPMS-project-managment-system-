			...
			<form action="" id="manage_file">
              ...
              <input type="date" class="form-control form-control-sm" autocomplete="off" name="issue_Date" value="<?php echo isset($issue_Date) ? date("Y-m-d",strtotime($issue_Date)) : '' ?>">
				...
				<div class="col-lg-12 text-right justify-content-center d-flex">
					...
					<button class="btn btn-secondary" type="button" onclick="location.href = 'index.php?page=bioPharma_list_file'">Cancel</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	$('#manage_file').submit(function(e){
		...
		// }
		$.ajax({
			url:'ajax.php?action=save_bioPharma_file',
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.replace('index.php?page=bioPharma_list_file')
					},750)
				}else if(resp == 2){
